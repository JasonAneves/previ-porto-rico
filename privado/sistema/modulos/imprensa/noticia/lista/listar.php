	...
	<form id="form_lista" name="form_lista" action="<?= $caminhoTela ?>" method="POST" onsubmit="return excluirVarios();">
		<div class="card-list">
			...
			if($_POST){
				...
				$data_noticia = filter_var($_POST['data_noticia']);
			...
			try {
				...
				if($data_noticia != "") {
					$sql .= "AND data_noticia BETWEEN :data_noticia1 AND :data_noticia2 ";
					$vetor['data_noticia1'] = formata_data_banco($data_noticia).' 00:00:00';
					$vetor['data_noticia2'] = formata_data_banco($data_noticia).' 23:59:59';
					$link .= "&data_noticia=" . urlencode($data_noticia);
				}

				if($tipo != "") {
					$sql .= "AND tipo LIKE :tipo ";
					$vetor['tipo'] = "%$tipo%";
					$link .= "&tipo=" . urlencode($tipo);
				}
