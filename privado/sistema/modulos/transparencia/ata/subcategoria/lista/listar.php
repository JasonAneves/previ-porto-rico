		...
		<div class="card-list">
			...
			try {
				$sql = "SELECT $tabela.*,
						  ...
						  FROM $tabela
					 LEFT JOIN ata_categoria ON $tabela.id_categoria = ata_categoria.id
				...
				if ($id_categoria != "") {
					$sql .= "AND ata_subcategoria.id_categoria = :id_categoria ";
					$vetor['id_categoria'] = $id_categoria;
					$link .= "&id_categoria=" . urlencode($id_categoria);
				}

				if ($descricao != "") {
					$sql .= "AND ata_subcategoria.descricao LIKE :descricao ";
