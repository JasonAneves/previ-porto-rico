<?php
  ...
  if($idExcluir) {
    ...
    excluir($_REQUEST['id_excluir'], $tb_nivel_2);
  }
?>
	...
	<form id="form_lista" name="form_lista" action="<?= $caminhoTela ?>" method="POST" onsubmit="return excluirVarios();">
      ...
      if($_GET['descricao'] != ''){
        $descricao = filter_var($_GET['descricao']);
			...
			try {
				$sql = "SELECT $tb_nivel_2.*,
                       $tb_nivel_1.descricao AS categoria
						      FROM $tb_nivel_2
					   LEFT JOIN $tb_nivel_1 ON $tb_nivel_2.id_nivel1 = $tb_nivel_1.id
						     WHERE $tb_nivel_2.status_registro = :status_registro
						       AND $tb_nivel_2.id_cliente = :id_cliente ";

        $vetor['id_cliente'] = $idCliente;

        if ($id_nivel1 != "") {
          $sql .= "AND ". $tb_nivel_2 .".id_nivel1 = :id_nivel1 ";
          $vetor['id_nivel1'] = $id_nivel1;
          $link .= "&id_nivel1=" . urlencode($id_nivel1);
        }

        if ($descricao != "") {
          $sql .= "AND ". $tb_nivel_2 .".descricao LIKE :descricao ";
        ...
        $totalRegistrosFilhos = $totalCadastrosFilhos + $totalCategoria_2Filhos;
