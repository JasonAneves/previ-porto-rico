...
try {
	...
	if(empty($id)) {
		$stCadastro = Conexao::chamar()->prepare("INSERT INTO $tb_nivel_3 
														 SET  id_cliente = :id_cliente, 
														  	  id_nivel2 = :id_nivel2,
															  descricao = :descricao,
															  link = :link_categoria");
		$stCadastro->bindValue("id_cliente", $idCliente, PDO::PARAM_INT);
		$stCadastro->bindValue("id_nivel2", $id_nivel2, PDO::PARAM_STR);
		...
		$stCadastro = Conexao::chamar()->prepare("UPDATE $tb_nivel_3 
		                                             SET id_nivel2 = :id_nivel2,
													 	 descricao = :descricao,
													 	 link = :link_categoria
												   WHERE id = :id ");
		$stCadastro->bindValue("id_nivel2", $id_nivel2, PDO::PARAM_STR);
