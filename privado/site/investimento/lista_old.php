                ...
                if (isset($url[$ii]) && strlen($url[$ii]) > 0 && $url[$ii] !== "pagina") {
                    ...
                    if ($total == 1) {
                      ...
                      echo "<div style='display: block;width: 100%;'><h1 class='acessibilidade' style=\" color: #0AAB60;
                        ...
                        try {
                            $qxCategoria = Conexao::chamar()->prepare("SELECT * FROM {$tabela}_categoria WHERE status_registro = :status_registro ORDER BY descricao");
                            $qxCategoria->bindValue(":status_registro", "A", PDO::PARAM_STR);
                            $qxCategoria->execute();

                            foreach ($qxCategoria->fetchAll(PDO::FETCH_ASSOC) as $investimento_categoria) {
                                ?>
                                <h3 class="acessibilidade">
                                    <?=$investimento_categoria['descricao']?>
                                </h3>	
                                <?php
                            
                                $qxSemSub = Conexao::chamar()->prepare("SELECT id, titulo, artigo FROM {$tabela} WHERE status_registro = :status_registro AND id_categoria = :id_categoria AND id_subcategoria IS NULL ORDER BY id DESC LIMIT $limit,$max");
                                $qxSemSub->bindValue(":status_registro", "A", PDO::PARAM_STR);
                                $qxSemSub->bindValue(":id_categoria", $investimento_categoria['id'], PDO::PARAM_INT);
                                $qxSemSub->execute();

                                foreach ($qxSemSub->fetchAll(PDO::FETCH_ASSOC) as $investimento) {
                                    ?>
                                    <a href="<?= $CAMINHO ?>investimento/<?= $investimento['id'] ?>" class="linha">
                                        <h5 class="acessibilidade"><i class="fa fa-chevron-circle-right" aria-hidden="true"></i><?= $investimento['titulo'] ?></h5>
                                    </a>	
                                    <?php
                                }
                                ...
                                $qxSubCategoria->bindValue(":id_categoria", $investimento_categoria['id'], PDO::PARAM_INT);
                                ...
                                foreach ($qxSubCategoria->fetchAll(PDO::FETCH_ASSOC) as $subcategoria) {
                                    ...
                                    <div class="panel panel-success">
                                        ...
                                        $qx->bindValue(":id_categoria", $investimento_categoria['id'], PDO::PARAM_INT);
                                        $qx->bindValue(":id_subcategoria", $subcategoria['id'], PDO::PARAM_INT);
                                        $qx->execute();

                                        foreach ($qx->fetchAll(PDO::FETCH_ASSOC) as $investimento) {
                                            ?>
                                            <a href="<?= $CAMINHO ?>investimento/<?= $investimento['id'] ?>" class="linha">
                                                <h5 class="acessibilidade"><i class="fa fa-chevron-circle-right" aria-hidden="true"></i><?= $investimento['titulo'] ?></h5>
                                            </a>	
                        ...
                        if ($total > $max) {
                            ...
                            <ul class="pagination pagination-lg">
                                <?php if ($at > 1) { ?>
                                    <li>
                                        <a href="<?= $CAMINHO ?>investimento/pagina/1">
                                            <i class="fa fa-angle-double-left"></i>
                                        </a>
                                    </li>
                                    <li>
                                        <a href="<?= $CAMINHO ?>investimento/pagina/<?= $at - 1 ?>">
                                ...
                                for ($x; $x <= $y; $x++) {
                                    ?>
                                    <li <?php if ($x == $at) echo "class='active'" ?>><a href="<?= $CAMINHO ?>investimento/pagina/<?= $x ?>"><?= $x ?></a></li>
                                <?php } ?>
                                <?php if ($at < $totalPag) { ?>		  
                                    <li>
                                        <a href="<?= $CAMINHO ?>investimento/pagina/<?= $at + 1 ?>">
                                    ...
                                    <li>
                                        <a href="<?= $CAMINHO ?>investimento/pagina/<?= $totalPag ?>">
