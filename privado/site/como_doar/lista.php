                    ...
                    if ($total == 1) {
                      ...
                      echo "<div style='display: block;width: 100%;'><h1 class='acessibilidade' style=\" color: #0AAB60;
                        ...
                        try {
                            ...
                            foreach ($qxCategoria->fetchAll(PDO::FETCH_ASSOC) as $como_doar_categoria) {
                                ?>
                                <h3>
                                    <?=$como_doar_categoria['descricao']?>
                                </h3>	
                                <?php
                                ...
                                $qxSemSub->bindValue(":id_categoria", $como_doar_categoria['id'], PDO::PARAM_INT);
                                $qxSemSub->execute();

                                foreach ($qxSemSub->fetchAll(PDO::FETCH_ASSOC) as $como_doar) {
                                    ?>
                                    <a href="<?= $CAMINHO ?>como_doar/<?= $como_doar['id'] ?>" class="linha">
                                        <h5 class="acessibilidade"><i class="fa fa-chevron-circle-right" aria-hidden="true"></i><?= $como_doar['titulo'] ?></h5>
                                    </a>	
                                    <?php
                                }
                                ...
                                $qxSubCategoria->bindValue(":id_categoria", $como_doar_categoria['id'], PDO::PARAM_INT);
                                ...
                                foreach ($qxSubCategoria->fetchAll(PDO::FETCH_ASSOC) as $subcategoria) {
                                    ...
                                    <div class="panel panel-success">
                                        ...
                                        $qx->bindValue(":id_categoria", $como_doar_categoria['id'], PDO::PARAM_INT);
                                        $qx->bindValue(":id_subcategoria", $subcategoria['id'], PDO::PARAM_INT);
                                        $qx->execute();

                                        foreach ($qx->fetchAll(PDO::FETCH_ASSOC) as $como_doar) {
                                            ?>
                                            <a href="<?= $CAMINHO ?>como_doar/<?= $como_doar['id'] ?>" class="linha">
                                                <h5 class="acessibilidade"><i class="fa fa-chevron-circle-right" aria-hidden="true"></i><?= $como_doar['titulo'] ?></h5>
                                            </a>	
                        ...
                        if ($total > $max) {
                            ...
                            <ul class="pagination pagination-lg">
                                <?php if ($at > 1) { ?>
                                    <li>
                                        <a href="<?= $CAMINHO ?>como_doar/pagina/1">
                                            <i class="fa fa-angle-double-left"></i>
                                        </a>
                                    </li>
                                    <li>
                                        <a href="<?= $CAMINHO ?>como_doar/pagina/<?= $at - 1 ?>">
                                ...
                                for ($x; $x <= $y; $x++) {
                                    ?>
                                    <li <?php if ($x == $at) echo "class='active'" ?>><a href="<?= $CAMINHO ?>como_doar/pagina/<?= $x ?>"><?= $x ?></a></li>
                                <?php } ?>
                                <?php if ($at < $totalPag) { ?>		  
                                    <li>
                                        <a href="<?= $CAMINHO ?>como_doar/pagina/<?= $at + 1 ?>">
                                    ...
                                    <li>
                                        <a href="<?= $CAMINHO ?>como_doar/pagina/<?= $totalPag ?>">
