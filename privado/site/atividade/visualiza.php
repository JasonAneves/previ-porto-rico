...
if($id){
	...
	if(count($registro['id']) > 0){
		...
		if(count($fotoArtigo['id']) > 0){
			$img = @getimagesize($CAMINHOIMG."/".$cliente."/".$fotoArtigo['foto']) ? $CAMINHOIMG."/".$cliente."/".$fotoArtigo['foto'] : $CAMINHOIMGCSS."/sem_foto.jpg";
			$img_light = @getimagesize($CAMINHOIMG."/".$cliente."/".$fotoArtigo['foto']) ? $CAMINHOIMG."/".$cliente."/gd_".$fotoArtigo['foto'] : $CAMINHOIMGCSS."/sem_foto.jpg";
		}
		
	?>
		<div class="box-visualizacao col-xs-12">

      <div style='display: block;width: 100%;'>
        <h1 style=" color: #33415C;font-size: 41px;font-weight: bold;font-family: 'Muli', sans-serif;">Modalidade / <?=$registro['titulo']?></h1>
			...
			<div class="panel panel-default">
				...
				<div class="panel-body">
					...
					foreach($fotos->fetchAll(PDO::FETCH_ASSOC) as $foto){
						...
						$img = getimagesize($CAMINHOIMG."/".$cliente."/".$foto['foto']) ? $CAMINHOIMG."/".$cliente."/tb_".$foto['foto'] : $CAMINHOIMGCSS."/sem_foto.jpg";
		
					?>
					<a href="<?=$img?>" data-toggle="lightbox" data-gallery="quem_somos_fotos" data-title="<?=$foto['legenda']?>">
						<img src="<?=$img?>" class="img-thumbnail foto-galeria" style="width: 30%;" alt="<?=$foto['legenda']?>"/>
					</a>
					<?php } ?>
				</div>
			</div>
		...
		<div class="outros_artigos">
			<h2>
				Veja Tamb&eacute;m
				<a href="<?=$CAMINHO?>/institucional" class="btn btn-padrao-verde pull-right"> <span style="color: #fff;" class="hidden-xs">VER TODOS</span></a>
			</h2>
			<?php 
			foreach($outros->fetchAll(PDO::FETCH_ASSOC) as $outros){
			?>
			<a href="<?=$CAMINHO?>/institucional/<?=$outros['id']?>" class="linha">
	...
	<?php 
		...
		echo "<script>location.href='".$CAMINHO."/atividade'</script>";
	}
