...
if ($id) {
    ...
    if (count($registro[id]) > 0) {
        ...
        $fotoArtigo = Conexao::chamar()->prepare("SELECT * FROM {$tabela}_foto WHERE id_noticia = :id_noticia ORDER BY id DESC LIMIT 1");
        $fotoArtigo->bindValue(":id_noticia", $registro[id], PDO::PARAM_INT);
        $fotoArtigo->execute();
        ...
        <div class="box-visualizacao">
            <a href="<?= $CAMINHO ?>/noticias" class="btn btn-success pull-right voltar"><i class="fa fa-angle-double-left"></i><span class="hidden-xs">VOLTAR</span></a>
            ...
            <div class="artigo-completo">
                <?php if (count($fotoArtigo[id]) > 0) { ?>
                    <a href="<?= $img ?>" data-toggle="lightbox" data-gallery="quem_somos_fotos" data-title="<?= $fotoArtigo['legenda'] ?>">
                        <img class="img-thumbnail foto-destaque-artigo" src="<?= $img ?>"/>
                    </a>
                <?php } ?>
                <?= formataArtigo($registro[artigo]) ?>
            </div>
            <?php
            $fotoArtigo[id] = isset($fotoArtigo[id]) ? $fotoArtigo[id] : 0;
            $verificaFotos = Conexao::chamar()->query("SELECT count(*) FROM {$tabela}_foto WHERE id_noticia = $registro[id] AND id != $fotoArtigo[id] ORDER BY id DESC")->fetchColumn();
            if ($verificaFotos > 0) {
                ?>
                <div class="panel panel-default">
                    ...
                    <div class="panel-body">
                        <?php
                        $fotos = Conexao::chamar()->query("SELECT * FROM {$tabela}_foto WHERE id_noticia = $registro[id] AND id != $fotoArtigo[id] ORDER BY id DESC");
            ...
            <?php } ?>
            <?php
            $verificaVideos = Conexao::chamar()->query("SELECT count(*) FROM {$tabela}_video WHERE id_noticia = $registro[id]")->fetchColumn();
            if ($verificaVideos > 0) {
                ?>
                <div class="panel panel-default">
                    <div class="panel-heading">
                    ...
                    <div class="panel-body">
                        <?php
                        $videos = Conexao::chamar()->query("SELECT * FROM {$tabela}_video WHERE id_noticia = $registro[id] ORDER BY id DESC");
            ...
            ?>
            <?php
            $verificaAnexos = Conexao::chamar()->query("SELECT count(*) FROM {$tabela}_anexo WHERE id_noticia = $registro[id]")->fetchColumn();
            if ($verificaAnexos > 0) {
                ?>
                <div class="panel panel-default">
                    <div class="panel-heading">
                    ...
                    <div class="panel-body">
                        <?php
                        $anexos = Conexao::chamar()->query("SELECT * FROM {$tabela}_anexo WHERE id_noticia = $registro[id] ORDER BY id DESC");
            ...
            <div class="outros_artigos">
                <h2>
                   Outras Publica&ccedil;&otilde;es
                    <a href="<?= $CAMINHO ?>/publicacoes" class="btn btn-success pull-right"><i class="fa fa-plus"></i> <span class="hidden-xs">VER TODOS</span></a>
                </h2>
                <?php
                foreach ($outros->fetchAll(PDO::FETCH_ASSOC) as $outros) {
                    ?>
                    <a href="<?= $CAMINHO ?>/publicacoes/<?= $outros['id'] ?>" class="linha">
        ...
        echo "<script>location.href='" . $CAMINHO . "/publicacoes'</script>";
    }
} else {
