...
function formataCep(valor) {
	const cep = valor.value.replace(/\D/g, '');
	if (cep !== "") {
...
function excluir_filhos(item, chave) {
	noty({
		text: chave ? 'Deseja realmente remover o registro? Digite a chave de seguran&ccedil;a:<br><input type="password" name="chave_excluir" id="chave_excluir" class="form-control" />' : 'Deseja realmente remover o registro?',
		type: 'alert',
		buttons: [{
			addClass: 'btn btn-primary', text: 'Sim', onClick: function ($noty) {
				if (chave) {
					if ($("#chave_excluir").val() == "") {
						...
						$.ajax({
							...
							success: function (data) {
								$noty.close();
								if (data == "sucesso") {
									jQuery(item).parent().fadeOut('slow', function () { jQuery(this).remove(); })
									$noty.close();
								} else {
									noty({ text: 'Chave de seguran&ccedil;a inv&aacute;lida!', type: 'error' });
								}
					...
					jQuery(item).parent().fadeOut('slow', function () {
						jQuery(this).remove();
						$noty.close();
					})
				}
...
function excluir_anexo(item, tipo) {
noty({
	text: 'Deseja realmente remover o arquivo? <br> Não pode ser desfeito.',
	type: 'alert',
	buttons: [{
		addClass: 'btn btn-primary', text: 'Sim', onClick: function($noty) {
			jQuery(item).parent().fadeOut('slow', function () { jQuery(this).remove(); })
...
function restaurar(tabela, id, link) {
	noty({
		text: 'Deseja realmente restaurar o registro?',
		type: 'alert',
		buttons: [{
			addClass: 'btn btn-primary', text: 'Sim', onClick: function($noty) {
				$.ajax({
...
function habilita_botao() {
	document.getElementById('botão_excluir').className = "btn btn-danger pull right";
}
