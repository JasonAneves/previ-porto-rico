<?php
    ...
    $link = "popup/transparencia_categoria_2.php?tela=$tela";

    if(empty($sort)) $sort = "descricao";
    if(empty($direction)) $direction = "ASC";

    if(empty($id_nivel1)) {
        echo "<script>$('#id_nivel1, #categoria_1').addClass('error');</script>";
        echo alerta("danger", "<i class=\"fa fa-ban\"></i> A Categoria não foi informada!");
    }
    if(empty($id_nivel2)) {
        echo "<script>$('#id_nivel2, #categoria_2').addClass('error');</script>";
        echo alerta("danger", "<i class=\"fa fa-ban\"></i> A Subcategoria não foi informada!");
    }
?>
