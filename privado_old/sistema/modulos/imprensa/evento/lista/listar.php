...
if($idExcluir) {
	...
	excluir($_REQUEST['id_excluir'], "evento");
}
?>
	...
	<form id="form_lista" name="form_lista" action="<?= $caminhoTela ?>" method="POST" onsubmit="return excluirVarios();">
		<div class="card-list">
			...
			if($_POST){
				...
				$data_evento = filter_var($_POST['data']);
			...
			try {
				...
				if($data_evento != "") {
					$sql .= "AND data BETWEEN :data_evento1 AND :data_evento2 ";
					$vetor['data_evento1'] = formata_data_banco($data_evento).' 00:00:00';
					$vetor['data_evento2'] = formata_data_banco($data_evento).' 23:59:59';
					$link .= "&data_evento=" . urlencode($data_evento);
				}
