    ...
    <div class="table-responsive hidden-sm hidden-xs">
        <table class="table table-striped table-hover table-bordered table-condensed">
            <tr class="active grid-topico">
                <th style="width: 90px;">
                    <?php if($totalCadastrosFilhos == 0 && (empty($statusRegistro) || $statusRegistro == "A")) { ?>
                        <input type="checkbox"
                            id="marcar" 
                            name="marcar" 
                ...
                <th>
                    <a  href="<?= $link ?>&tela=<?= $tela ?>&sort=transparencia_subcategoria.descricao&direction=<?= empty($direction) || $direction == "DESC" ? "ASC" : "DESC" ?>" 
                        data-toggle="tooltip" 
                        data-placement="top" 
                        title="Clique para ordenar">
                        Descrição
                    </a>
                    <?php if($sort == "transparencia_subcategoria.descricao" && $direction == "ASC") { ?><i class="fa fa-caret-up"></i><?php } ?>
                    <?php if($sort == "transparencia_subcategoria.descricao" && $direction == "DESC") { ?><i class="fa fa-caret-down"></i><?php } ?>
                </th>
                <th>
                    <a  href="<?= $link ?>&tela=<?= $tela ?>&sort=transparencia_categoria.descricao&direction=<?= empty($direction) || $direction == "DESC" ? "ASC" : "DESC" ?>" 
                        data-toggle="tooltip" 
                        data-placement="top" 
                        title="Clique para ordenar">
                        Categoria
                    </a>
                    <?php if($sort == "transparencia_categoria.descricao" && $direction == "ASC") { ?><i class="fa fa-caret-up"></i><?php } ?>
                    <?php if($sort == "transparencia_categoria.descricao" && $direction == "DESC") { ?><i class="fa fa-caret-down"></i><?php } ?>
