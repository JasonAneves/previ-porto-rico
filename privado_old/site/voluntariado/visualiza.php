...
if($id){
	...
	?>
      ...
      <div style='display: block;width: 100%;'>
        <h1 class="acessibilidade" style=" color: #0aab60;font-size: 35px;font-weight: bold;font-family: 'Muli', sans-serif;">Voluntariado / <?=$registro['titulo']?></h1>
      </div>
		...
		<div class="outros_artigos">
			<h2>
				Veja Tamb&eacute;m
				<a href="<?=$CAMINHO?>/voluntariado" class="btn btn-padrao-verde pull-right"> <span style="color: #fff;" class="hidden-xs">VER TODOS</span></a>
			</h2>
			<?php 
			foreach($outros->fetchAll(PDO::FETCH_ASSOC) as $outros){
			?>
			<a href="<?=$CAMINHO?>voluntariado/<?=$outros['id']?>" class="linha">
	...
	<?php 
		...
		echo "<script>location.href='".$CAMINHO."/voluntariado'</script>";
	}
