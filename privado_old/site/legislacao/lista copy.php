<div class="row" style="margin:0;">
    <div class="container" style="min-height: 500px;">
			<h2 style="color: #0aab60;font-size: 35px;font-weight: bold;font-family: 'Muli', sans-serif;">Legislação</h2>
<?php
		$qryCat = Conexao::chamar()->prepare("SELECT * FROM legislacao_categoria WHERE status_registro = 'A' ORDER BY descricao DESC");
		$qryCat->execute();
    ...
    foreach($BuscaCat as $cat){
			...
			$qryArtigo = Conexao::chamar()->prepare("SELECT * FROM legislacao_subcategoria WHERE id_categoria = '$cat[id]' AND status_registro = 'A' ORDER BY id DESC");
			$qryArtigo->execute();
	            ...
	            <div class="panel panel-default">
	                ...
	                <div id="<?=$count?>" class="panel-collapse collapse">
	                    <div class="panel-body">
												<!-- <?php if ($busca_artigo["artigo"] != ""){ ?>
														<div class="acessibilidade" style="font-weight: 600; font-size:15px; padding:10px;"><?=resumo_artigo($busca_artigo["artigo"],300) . '<br>';?></div>
												<?php }  ?> -->
										 <?php $qryAnexo = Conexao::chamar()->prepare("SELECT * FROM legislacao WHERE id_artigo='$busca_artigo[id]' ORDER BY ordem ASC, id DESC") or die(mysql_error());
												$qryAnexo->execute();
