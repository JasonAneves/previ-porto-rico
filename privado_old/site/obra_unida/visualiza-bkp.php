...
if($id){
	...
	if(isset($registro['id'])){
		...
		if(isset($fotoArtigo['id'])){
			...
			$img_light = @getimagesize($CAMINHOIMG."/".$fotoArtigo['foto']) ? $CAMINHOIMG."/".$fotoArtigo['foto'] : $CAMINHOIMGCSS."/sem_foto.jpg";
		}
	...
	?>
      ...
      <div style='display: block;width: 100%;'>
        <h1 class="acessibilidade" style=" color: #0aab60;font-size: 35px;font-weight: bold;font-family: 'Muli', sans-serif;">Entidades / <?=$registro['nome']?></h1>
      </div>
		...
		<div class="outros_artigos">
			<h2>
				Veja Tamb&eacute;m
				<a href="<?=$CAMINHO?>/obra_unida" class="btn btn-padrao-verde pull-right"> <span style="color: #fff;" class="hidden-xs">VER TODOS</span></a>
			</h2>
			<?php 
			foreach($outros->fetchAll(PDO::FETCH_ASSOC) as $outros){
			?>
			<a href="<?=$CAMINHO?>obra_unida/<?=$outros['id']?>" class="linha">
				<div class="nome"><?=$outros['nome']?></div>	
	...
	<?php 
		...
		echo "<script>location.href='".$CAMINHO."/obra_unida'</script>";
	}
