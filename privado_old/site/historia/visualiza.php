...
if($id){
	...
	if(count($registro['id']) > 0){
		...
		if(count($fotoArtigo['id']) > 0){
			$img = @getimagesize($CAMINHOIMG."/".$cliente."/".$fotoArtigo['foto']) ? $CAMINHOIMG."/".$cliente."/tb_".$fotoArtigo['foto'] : $CAMINHOIMGCSS."/sem_foto.jpg";
	...
	?>
      ...
      <div style='display: block;width: 100%;'>
        <h1 class="acessibilidade" style=" color: #33415C;font-size: 41px;font-weight: bold;font-family: 'Muli', sans-serif;">História / <?=$registro['titulo']?></h1>
      </div>
			...
			<div class="panel panel-default">
				...
				<div class="panel-body">
					...
					foreach($fotos->fetchAll(PDO::FETCH_ASSOC) as $foto){
						...
						$img = getimagesize($CAMINHOIMG."/".$cliente."/".$foto['foto']) ? $CAMINHOIMG."/".$cliente."/gd_".$foto['foto'] : $CAMINHOIMGCSS."/sem_foto.jpg";
	...
	<?php 
		...
		echo "<script>location.href='".$CAMINHO."/historia'</script>";
	}
