<?php
  switch ($url[$ii]){
    case 'censo': $servico = 'censo'; $titulo = 'Censo'; break;
    case 'controle_social': $servico = 'controle_social'; $titulo = 'Controle Social'; break;
    case 'planejamento': $servico = 'planejamento'; $titulo = 'Planejamento'; break;
  }

  $categoria = $servico . '_categoria'; 
  $subcategoria = $servico . '_subcategoria';
  $servicoFoto = $servico . '_foto';
  $servicoVideo = $servico . '_video';
  $servicoAnexo = $servico . '_anexo';
?>
<div class="row" style="margin:0;">
    ...
    <?php
		...
		$qryCat = Conexao::chamar()->prepare("SELECT * 
    FROM $categoria 
    ...
    foreach($BuscaCat as $cat){ ?>
      ...
      <?php 
        $qryAnexoSemSub = Conexao::chamar()->prepare("SELECT * 
        FROM $servico
        WHERE id_cliente = '$idCliente' 
        AND id_categoria = '$cat[id]' 
      ...
      <?php } ?>
			...
			<?php 
      $qryArtigo = Conexao::chamar()->prepare("SELECT * 
      FROM $subcategoria
	            ...
	            <div class="panel panel-default">
	                ...
	                <div id="sub<?=$i?>" class="panel-collapse collapse">
	                    ...
	                    <div class="panel-heading" style="border: 2px solid aliceblue;padding: 10px 0;margin-bottom: 10px;margin-top:-10px;">
												<?php
                          $qrySub = Conexao::chamar()->prepare("SELECT * 
                          FROM $servico
                          WHERE id_cliente = '$idCliente' 
                          AND id_categoria = '$cat[id]' 
                          AND id_subcategoria = '$busca_artigo[id]' 
                          AND status_registro = 'A' 
                          ORDER BY id DESC");
                          $qrySub->execute();
                        ...
                        ?>
                          ...
                          <div id="anexo<?=$i.$ii?>" class="panel-collapse collapse acessibilidade" style="padding:10px; border: 1px solid aliceblue;">
                            ...
                            <?php
                              $qryFoto = Conexao::chamar()->prepare("SELECT * 
                              FROM $servicoFoto 
                              WHERE id_artigo = '$busca_sub[id]' 
                              ORDER BY id DESC");
                              $qryFoto->execute();
                            ...
                            <?php } 
                              ...
                              $qryVideo = Conexao::chamar()->prepare("SELECT * 
                              FROM $servicoVideo
                              WHERE id_artigo = '$busca_sub[id]' 
                              ORDER BY id DESC");
                              $qryVideo->execute();
                            ...
                            <?php } 
                              $qryAnexo = Conexao::chamar()->prepare("SELECT * 
                              FROM $servicoAnexo
                              WHERE id_artigo = '$busca_sub[id]' 
                              ORDER BY id DESC");
                              $qryAnexo->execute();
