...
if (!empty($_POST)) {
    ...
    if (empty($_POST['nome'])) {
        ...
        $subject = "Instituto de Previdência - Ouvidoria";

        $html  = "<html><body>";
        $html .= "<p>" . date("d/m/Y H:i") . "</p>";
        $html .= "<p><strong>Identificação: </strong>";
        $html .= "$_POST[identificacao]</p>";
    ...
    <div class="container">
        <div class="col-md-12">
            <div style="display: block;width: 100%;margin: 20px; 0">
                <h2 style="color: #007CC2;font-size: 35px;font-weight: bold;font-family: 'Muli', sans-serif;">Ouvidoria</h2>
            ...
            <form class="form-horizontal" name="contato" action="" enctype="multipart/form-data" method="POST">
                <div class="form-group">
                    <label for="identificacao" class="col-sm-2 control-label">Identifica&ccedil;&atilde;o</label>
                    <div class="col-sm-10">

                        <select name="identificacao" id="identificacao" class="form-control required" required>
                            <option value="aberto" selected>Aberta</option>
                            <option value="sigiloso">Sigilosa</option>
                        </select>
                        <p id="desc_aberto" style="margin-top:10px;">As informações abaixo ficarão disponíveis durante a tramitação da manifestação, permitindo maior agilidade na apuração.</p>
                        <p id="desc_sigiloso" style="display:none; margin-top:10px;">As informações abaixo ficarão disponíveis para a Ouvidoria, mantendo-se o sigilo durante a tramitação da manifestação.</p>
...
<script>
    $("select[name=identificacao]").change(function() {
        if ($(this).children(':selected').val() == 'aberto') {
            $("#desc_aberto").show('fast');
        } else {
            $("#desc_aberto").hide('fast');
        }

        if ($(this).children(':selected').val() == 'sigiloso') {
            $("#desc_sigiloso").show('fast');
        } else {
            $("#desc_sigiloso").hide('fast');
        }

        if ($(this).children(':selected').val() == 'anonimo') {
            $(".esconde_anonimo").hide('fast');
            $("#desc_anonimo").show('fast');
        } else {
            $(".esconde_anonimo").show('fast');
            $("#desc_anonimo").hide('fast');
        }
    });
</script>