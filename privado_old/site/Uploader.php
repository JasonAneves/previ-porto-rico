...
class Uploader {
		...
		foreach($files as $file_field) {
			...
			if($file_field['error'] == UPLOAD_ERR_OK) {
				...
				if(preg_match("/\.($this->pattern){1}$/i", $file_field['name'], $ext)) {
					...
					$ext = array_pop(explode('.',$file_field['name']));
					...
					if($conf_zip) {
						...
						if(!file_exists(dirname($cannonical))) {
							...
							chmod(dirname($cannonical), 01777);
						}

						if(!move_uploaded_file($file_field['tmp_name'], $cannonical)) {
							//$erro = 'Falha na gravacao do arquivo!';
							if(!is_writable($this->dir.$this->file)) {
								$erro .= ' - Nao foi possivel escrever na pasta definitiva!';
							}

							if(!is_readable($file_field['tmp_name'])) {
								//$erro .= ' - Nao foi possivel ler da pasta temporaria!';
							}
						} else {
						...
						$erro = "O arquivo zip enviado possui formatos de arquivos invalidos que podem comprometer a seguranca!";
					}
				} else {

					$erro = "O tipo de arquivo " . strtoupper(array_pop(explode('.',$file_field['name']))) . " e invalido e pode comprometer a seguranca do sistema!";
				}
			} else {
				switch ($file_field['error']) {
					...
					case UPLOAD_ERR_NO_TMP_DIR:
						$erro .= 'Falha no upload do arquivo! - Nao foi localizada uma pasta temporaria!';
						break;
					case UPLOAD_ERR_CANT_WRITE:
						$erro .= 'Falha no upload do arquivo! - Nao foi possivel escrever na pasta temporaria!';
						break;
	...
	public function uploadImage($wgd = "",$hgd = "",$wtb = "",$htb = "") {
		...
		foreach($file as $afile) {
			...
			if($this->remove) {
				if($multi_field) throw new Exception('Nao e possivel remover arquivos de campos multivalorados!');
